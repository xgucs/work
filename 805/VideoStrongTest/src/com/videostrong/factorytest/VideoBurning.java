...
public class VideoBurning extends Activity {  
	...
	*/
		...
		time=Long.valueOf(SystemProperties.get("persist.sys.time","0"));
		...
		counter.schedule(new TimerTask() {
			...
			@Override
			public void run() {
				SystemProperties.set("persist.sys.time",String.valueOf(time));
			}
		}, 0, 5000);
		...
		led_control.schedule(new TimerTask() {
			...
			public void run() {
				if (current.equals("1")) {
				 CommonUtilits.writeLed("/sys/devices/platform/aml_pm_m8/led_test","0");
				 current="0";
				}else if(current.equals("0")){
					  CommonUtilits.writeLed("/sys/devices/platform/aml_pm_m8/led_test","1");
                      current="1";
				}			   
				}
		}, 0,500);
